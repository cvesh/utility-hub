...
export default function Base64Tool() {
  ...
  const handleConvert = () => {
    ...
    if (!currentInput) { setOutput(''); return; }
    try {
      setOutput(isEncoding ? base64Encode(currentInput) : base64Decode(currentInput));
      setError('');
    } catch (e) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-xl mx-auto my-8">
      ...
      <textarea value={input} onChange={handleInputChange} placeholder={isEncoding ? 'Enter text to encode...' : 'Enter Base64 to decode...'} rows={4} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none resize-y font-mono text-sm mb-3" />
      ...
      <div className="relative">
        ...
        {output && <button onClick={copyToClipboard} className="absolute top-2 right-2 px-2 py-1 text-xs bg-blue-600 text-white rounded hover:bg-blue-700">Copy</button>}
      </div>
    </div>
  );
}